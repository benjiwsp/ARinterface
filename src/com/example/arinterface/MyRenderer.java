...
public class MyRenderer implements Renderer {
	...
	Words time;
	Words twitterText;
	String nametag = "@VisualEyes_";
	JsonRetriever words;

	public MyRenderer(final Context activityContext,
			SampleApplicationSession session, WebView webPreview) {
		mActivityContext = activityContext;
		vuforiaAppSession = session;
		this.webPreview = webPreview;
		initWebviewclient();
		webPreview
				.loadUrl("http://marvel.com/movies/movie/181/captain_america_the_winter_soldier");
		words = new JsonRetriever();
		words.retrieveTwitterData("VisualEyes_");
	...
	public void onDrawFrame(GL10 gl) {
		...
		if (state.getNumTrackableResults() > 0) {
			...
			if (trackable.getName().equalsIgnoreCase("chips")) {
				...
				GLES20.glUniform1f(GLES20.glGetUniformLocation(
						shaderPrograms.get(0).programHandle, "fading"), 0.6f);
				square.draw(projection, mv, shaderPrograms.get(0),
						frameTexture[0]);
				GLES20.glUniform1f(GLES20.glGetUniformLocation(
						shaderPrograms.get(0).programHandle, "fading"), 0.0f);
				...
				mv = modelViewMatrix_Vuforia.getData();
				Matrix.translateM(mv, 0, -430.0f, 300.0f, 5.0f);
				...
				updateDate();
				mv = modelViewMatrix_Vuforia.getData();
				Matrix.translateM(mv, 0, -430.0f, 250.0f, 5.0f);
				...
				// draw chat
				mv = modelViewMatrix_Vuforia.getData();
				Matrix.translateM(mv, 0, -500.0f, -250.0f, 4.0f);
				Matrix.scaleM(mv, 0, 500.0f, 250.0f, 1.0f);
				GLES20.glEnable(GLES20.GL_BLEND);
				square.draw(projection, mv, shaderPrograms.get(0),
						chatTexture[0]);
				GLES20.glDisable(GLES20.GL_BLEND);
				...
				if (displayTwitterText) {
					mv = modelViewMatrix_Vuforia.getData();
					Matrix.translateM(mv, 0, -460.0f, -40.0f, 5.0f);
					Matrix.scaleM(mv, 0, 12.3f, 12.3f, 1.0f);
					GLES20.glEnable(GLES20.GL_BLEND);
					twitterText.draw(projection, mv, shaderPrograms.get(0),
							texTexture[0]);
					GLES20.glDisable(GLES20.GL_BLEND);
				}
				...
				// make frame
				Matrix.scaleM(mv, 0, 1200.0f, 750.0f, 1.0f);
				Matrix.translateM(mv, 0, -0.5f, -0.5f, -1.0f);
				GLES20.glEnable(GLES20.GL_BLEND);
				GLES20.glUniform1f(GLES20.glGetUniformLocation(
						shaderPrograms.get(0).programHandle, "fading"), 0.6f);
				...
				// Matrix.translateM(mv, 0, -5.0f, 300.0f, 5.0f);
				Matrix.scaleM(mv, 0, 300.0f, 50.0f, 1.0f);
				Matrix.translateM(mv, 0, -1.75f, 6.0f, 5.0f);
				GLES20.glEnable(GLES20.GL_BLEND);
				square.draw(projection, mv, shaderPrograms.get(0),
						chromeTexture[0]);
				GLES20.glDisable(GLES20.GL_BLEND);
				...
				mv = modelViewMatrix_Vuforia.getData();
				Matrix.scaleM(mv, 0, ratio * 850.0f, 650.0f, 1.0f);
				Matrix.translateM(mv, 0, -1.0f, -0.55f, 5.0f);
				if (previewTexture != null)
					square.draw(projection, mv, shaderPrograms.get(0),
							previewTexture[0]);
				else
				...
				mv = modelViewMatrix_Vuforia.getData();
				Matrix.translateM(mv, 0, 20.0f, 240.0f, 5.0f);
				Matrix.scaleM(mv, 0, 50.0f, 50.0f, 1.0f);
				GLES20.glEnable(GLES20.GL_BLEND);
				time.draw(projection, mv, shaderPrograms.get(0), texTexture[0]);
				GLES20.glDisable(GLES20.GL_BLEND);
				...
				updateDate();
				mv = modelViewMatrix_Vuforia.getData();
				Matrix.translateM(mv, 0, 25.0f, 200.0f, 5.0f);
				Matrix.scaleM(mv, 0, 25.0f, 25.0f, 1.0f);
				GLES20.glEnable(GLES20.GL_BLEND);
				time.draw(projection, mv, shaderPrograms.get(0), texTexture[0]);
				GLES20.glDisable(GLES20.GL_BLEND);
				...
				// draw chat
				mv = modelViewMatrix_Vuforia.getData();
				Matrix.translateM(mv, 0, 25.0f, -200.0f, 7.0f);
				Matrix.scaleM(mv, 0, 500.0f, 350.0f, 1.0f);
				GLES20.glEnable(GLES20.GL_BLEND);
				square.draw(projection, mv, shaderPrograms.get(0), weather[0]);
				GLES20.glDisable(GLES20.GL_BLEND);
	...
	public void onSurfaceCreated(GL10 gl, EGLConfig config) {
		...
		defaultTexture = TextureHelper.loadTexture(mActivityContext,
				R.drawable.questionmark);
		texTexture = TextureHelper.loadTexture(mActivityContext,
				R.drawable.text);
		frameTexture = TextureHelper.loadTexture(mActivityContext,
				R.drawable.background2);
		chromeTexture = TextureHelper.loadTexture(mActivityContext,
				R.drawable.chrome);
		twitterTexture = TextureHelper.loadTexture(mActivityContext,
				R.drawable.twitter);
		chatTexture = TextureHelper.loadTexture(mActivityContext,
				R.drawable.border);
		background2 = TextureHelper.loadTexture(mActivityContext,
				R.drawable.tree);
		weather = TextureHelper.loadTexture(mActivityContext,
				R.drawable.weather);
		square = new Square();
	...
	public void initBasicHandles(ShaderHandles shader) {
		// attributes
		shader.positionHandle = GLES20.glGetAttribLocation(
				shader.programHandle, "aPosition");
		shader.mTextureCoordinateHandle = GLES20.glGetAttribLocation(
				shader.programHandle, "aTexCord");

		// uniforms
		shader.mvHandle = GLES20.glGetUniformLocation(shader.programHandle,
				"mv");
		shader.projectionHandle = GLES20.glGetUniformLocation(
				shader.programHandle, "projection");
		shader.mTextureUniformHandle.add(GLES20.glGetUniformLocation(
				shader.programHandle, "uTexture"));
	}
	...
	public void initWebviewclient() {
		...
		webPreview.setWebViewClient(new WebViewClient() {
			...
			public void onPageFinished(WebView view, String url) {
				...
				if (loadingFinished && !redirect) {
					preview = (loadWebBitmap) new loadWebBitmap().execute(view
							.getScrollY());
				} else {
					redirect = false;
				}
		 ...
		 * This method of getting a bitmap requires the user to download the
		 * webpage even if the user only wants to see metadata on a service.
		 */
	...
	public class loadWebBitmap extends AsyncTask<Integer, Void, Integer> {
		...
		protected Integer doInBackground(Integer... params) {
			try {
				Thread.sleep(500);
				Bitmap b = Bitmap.createBitmap(webPreview.getWidth(),
						webPreview.getHeight(), Bitmap.Config.ARGB_4444);
	...
	public void updateTime() {
		...
		now.setToNow();
		textCreator.newWord();
		textCreator.addWords(Integer.toString(now.hour) + ":"
	...
	public void scrollCounter() {
		...
		if (elapsedtime > 0.5) {
			...
			if (canScroll == true) {
				/*
				 * webPreview.scrollTo(0, scrollPosition); Bitmap b =
				 * Bitmap.createBitmap(webPreview.getWidth(),
				 * webPreview.getHeight(), Bitmap.Config.ARGB_4444);
				 * webPreview.setDrawingCacheEnabled(true); b =
				 * Bitmap.createBitmap(webPreview.getDrawingCache());
				 * webPreview.setDrawingCacheEnabled(false); //webpage =
				 * b.copy(b.getConfig(), false); scrollPosition += 100;
	...
	public void createTwitterText() {
		try {
			...
			textCreator.newWord();
			textCreator.addWords("         " + nametag);
			textCreator.addWords("");
			int count = 0;
			for (int x = 0; x + 5 <= tokens.length - 1; x += 5) {
				textCreator.addWords(tokens[x] + " " + tokens[x + 1] + " "
