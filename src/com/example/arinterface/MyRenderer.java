...
public class MyRenderer implements Renderer {
	...
	public MyRenderer(final Context activityContext,SampleApplicationSession session, WebView webPreview)
	{
		...
		initWebviewclient();
		webPreview.loadUrl("http://marvel.com/movies/movie/181/captain_america_the_winter_soldier");
	...
	public void onDrawFrame(GL10 gl) {
		...
		//make web
			...
			GLES20.glEnable(GLES20.GL_BLEND);
			square.draw(projection, mv, shaderPrograms.get(0), chatTexture[0]);
			GLES20.glDisable(GLES20.GL_BLEND);
			...
			GLES20.glEnable(GLES20.GL_BLEND);
			GLES20.glUniform1f(GLES20.glGetUniformLocation(shaderPrograms.get(0).programHandle, "fading"),0.6f);
			square.draw(projection, mv, shaderPrograms.get(0),background2[0]);
			GLES20.glUniform1f(GLES20.glGetUniformLocation(shaderPrograms.get(0).programHandle, "fading"),0.0f);
			GLES20.glDisable(GLES20.GL_BLEND);
			mv = modelViewMatrix_Vuforia.getData();
		//make web
	...
	public void onSurfaceCreated(GL10 gl, EGLConfig config) {
		...
		frameTexture = TextureHelper.loadTexture(mActivityContext,R.drawable.background2);
		...
		chatTexture = TextureHelper.loadTexture(mActivityContext,R.drawable.border);
		background2 = TextureHelper.loadTexture(mActivityContext,R.drawable.tree);
		weather = TextureHelper.loadTexture(mActivityContext,R.drawable.weather);
		square = new Square();
	...
	public void createTwitterText()
	{
		try {
			...
			{
				textCreator.addWords(tokens[x]+" "+tokens[x+1]+" "+tokens[x+2]+" "+tokens[x+3]+" "+tokens[x+4]+" ");
				count +=5;
			}
			String stringEnd = "";
